...
void create_abonents(int&);//создание новых сотрудниц
void out();//Вывод данных о сотрудницах и их детях на консоль
void sort();//Меню сортировки, различые виды сортировок
void kolxxxx();//Количество абонент
void table(int&);//Таблица 
void update_abonents(int&);//Редактировать сотрудниц
void delete_abonents(int&);//Удаление записи сотрудницы
void search_abonents();//Разные виды поиска сотрудниц
	...
	if (i == 1)
	{
		if (vecAbonents.size() != 0)
		{
			...
			case '1': {
				...
				cout << "\nВведите изменённое фамилию сотрудницы\n";
...
void delete_abonents(int& i)
{
	if (i == 1)
	{
		if (vecAbonents.size() != 0)
		{
			string tmp;
			out();
			cout << "\nКакую запись сотрудника вы хотите удалить?\n";
			int n = proverka();
